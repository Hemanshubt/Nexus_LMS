...
import { Star, Users, Award, ShieldCheck, Zap } from "lucide-react";
            ...
            <section className="relative overflow-hidden bg-background py-20 lg:py-32">
                <div className="container relative z-10 flex flex-col items-center gap-8 text-center">
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-6 mt-4">
                        <Link to="/register">
                            <Button size="lg" className="h-16 px-10 text-xl font-bold rounded-2xl shadow-2xl shadow-primary/20 transition-all hover:scale-105 active:scale-95">
                                Start Your Odyssey
                            </Button>
                        </Link>
                        <Link to="/courses">
                            <Button size="lg" variant="outline" className="h-16 px-10 text-xl font-bold rounded-2xl border-2 hover:bg-muted transition-all">
                                Exploration Path
                            </Button>
                        </Link>
                    </div>
            ...
            <section className="bg-muted/30 py-24">
                <div className="container space-y-16">
                    ...
                    <div className="grid gap-8 md:grid-cols-3">
                        <Card className="border-none bg-transparent shadow-none text-center space-y-4">
                ...
                <div className="container space-y-12">
                    <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                        ...
                        <Button variant="ghost" asChild>
                            <Link to="/courses">View all courses &rarr;</Link>
                        </Button>
                    </div>
                    ...
                    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                        {courses.length === 0 ? (
                            Array(3).fill(0).map((_, i) => (
                                <Card key={i} className="animate-pulse flex flex-col h-[400px]">
                            ...
                            courses.map(course => (
                                <Card key={course.id} className="group flex flex-col overflow-hidden transition-all hover:shadow-xl hover:-translate-y-1">
                                    ...
                                    <CardContent className="px-6 flex-1 pt-0">
                                        <div className="flex items-center gap-1 text-yellow-500 text-sm font-bold">
                                            <span>{course.avgRating?.toFixed(1) || "0.0"}</span>
                                            <div className="flex">
                                                {[1, 2, 3, 4, 5].map((s) => (
                                                    <Star key={s} className={`h-3 w-3 ${Number(course.avgRating || 0) >= s ? 'fill-current' : 'text-muted'}`} />
                                                ))}
                                            </div>
                ...
                <div className="rounded-3xl bg-primary px-6 py-20 text-center text-primary-foreground md:px-12 lg:py-32 overflow-hidden relative">
                    <div className="relative z-10 space-y-8">
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-4">
                            <Button variant="secondary" size="lg" className="rounded-full px-8">Join Now</Button>
                            <Button variant="ghost" className="text-primary-foreground hover:bg-white/10 rounded-full px-8" asChild>
            ...
            <footer className="border-t py-12 md:py-24">
                <div className="container flex flex-col gap-8 md:flex-row md:justify-between">
                    <div className="space-y-4 max-w-sm">
                        <Link to="/" className="text-2xl font-bold tracking-tight">nexus</Link>
                    ...
                    <div className="grid grid-cols-2 gap-12 sm:grid-cols-3">
                        <div className="space-y-4">
                            ...
                            <ul className="space-y-2 text-sm text-muted-foreground">
                                ...
                                <li><Link to="/dashboard" className="hover:text-primary">My Learning</Link></li>
                                <li><Link to="/instructor" className="hover:text-primary">Teach on Nexus</Link></li>
                            </ul>
                        </div>
                        <div className="space-y-4">
