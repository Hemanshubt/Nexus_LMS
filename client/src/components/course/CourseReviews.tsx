        ...
        <div className="space-y-8">
            <div className="flex items-center gap-6">
                <div className="text-center">
                    ...
                    <div className="flex justify-center my-2">
                        {[1, 2, 3, 4, 5].map((s) => (
                            <Star key={s} className={`w-4 h-4 ${Number(avgRating) >= s ? 'fill-primary text-primary' : 'text-muted'}`} />
                        ))}
                    </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="flex gap-2">
                                {[1, 2, 3, 4, 5].map((s) => (
                                    ...
                                    >
                                        <Star className={`w-8 h-8 ${rating >= s ? 'fill-primary text-primary' : 'text-muted-foreground hover:text-primary/50'}`} />
                                    </button>
                                ))}
                            </div>
                            <Textarea
            ...
            <div className="space-y-6">
                ...
                <div className="grid gap-6">
                    {reviews.length === 0 ? (
                        <p className="text-muted-foreground text-center py-8 border rounded-lg border-dashed">No reviews yet. Be the first to leave one!</p>
                    ) : (
                        reviews.map((review) => (
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <div className="flex-1 space-y-1">
                                        ...
                                        <div className="flex mb-2">
                                            {[1, 2, 3, 4, 5].map((s) => (
                                                <Star key={s} className={`w-3 h-3 ${review.rating >= s ? 'fill-yellow-400 text-yellow-400' : 'text-muted'}`} />
                                            ))}
                                        </div>
