...
export const getAssignment = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
export const manageAssignment = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { lessonId } = req.params;
...
export const submitAssignment = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { assignmentId } = req.params;
    const userId = req.user.id;
    const { fileUrl } = req.body;

    if (!fileUrl) {
        return next(new AppError('Please provide a file URL', 400));
    }
